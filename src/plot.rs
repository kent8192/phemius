...
pub fn validate_structure(value: &StoryStructure) -> Result<()> {
	validate_unique_ids("part", value.parts.iter().map(|part| &part.id))?;
	validate_unique_ids("chapter", value.chapters.iter().map(|chapter| &chapter.id))?;
	validate_unique_ids("scene", value.scenes.iter().map(|scene| &scene.id))?;
	validate_unique_ids("box", value.boxes.iter().map(|box_| &box_.id))?;
